...
async function NASCMiddleware(request: express.Request, response: express.Response, next: express.NextFunction): Promise<void> {
	...
	const allowLocalBannedDevices = process.env.PN_ACT_CONFIG_ALLOW_LOCAL_BANNED_DEVICES === 'true';
	...
	LOG_INFO(`[NASC][LOCAL-3DS] action=${action} title=${titleID} pid=${pid || 'none'} serial=${serialNumber || 'none'} certValid=${cert.valid} allowLocal=${allowLocal3DS}`);

	if (!cert.valid && !allowLocal3DS) {
		LOG_WARN(`[NASC][LOCAL-3DS] rejecting invalid certificate with 121 for title=${titleID}`);
		response.status(200).send(nascError('121').toString());
		return;
	}
	if (!cert.valid && allowLocal3DS) {
		LOG_INFO('[NASC][LOCAL-3DS] accepting placeholder certificate for private local server');
	}

	if (!validNintendoMACAddress(macAddress) && !allowLocal3DS) {
		response.status(200).send(nascError('null').toString());
		return;
	}
	if (!validNintendoMACAddress(macAddress) && allowLocal3DS) {
		LOG_INFO('[NASC][LOCAL-3DS] accepting non-Nintendo MAC for private local server');
	}
	...
	if (pid) {
		...
		if (nexAccount && nexAccount.access_level < 0 && allowLocalBannedDevices) {
			LOG_WARN(`[NASC][LOCAL-3DS] clearing local NEX account ban for pid=${nexAccount.pid}`);
			nexAccount.access_level = 0;
			nexAccount.server_access_level = 'prod';
	...
	if (!nexAccount && allowLocal3DS) {
		nexAccount = await NEXAccount.findOne({ access_level: { $gte: 0 } });
		if (nexAccount) {
			pid = nexAccount.pid;
			LOG_INFO(`[NASC][LOCAL-3DS] using fallback local NEX account pid=${pid}`);
		} else {
			LOG_WARN('[NASC][LOCAL-3DS] no local NEX account found; NASC route may return null');
		}
	}
	...
	if (device) {
		if (device.access_level < 0 && allowLocalBannedDevices) {
			LOG_WARN(`[NASC][LOCAL-3DS] clearing local device ban for serial=${device.serial}`);
		...
		if (device.access_level < 0) {
			LOG_WARN(`[NASC][LOCAL-3DS] rejecting device ban with 102 for serial=${device.serial}`);
			response.status(200).send(nascError('102').toString());
			return;
		}
		...
		if (device.serial !== serialNumber && allowLocal3DS) {
			device.serial = serialNumber;
			device.fcdcert_hash = fcdcertHash;
			device.mac_hash = macAddressHash;
			device.access_level = 0;
			device.server_access_level = 'prod';
			await device.save();
		}
	...
	if (!device && pid) {
		LOG_INFO(`[NASC][LOCAL-3DS] creating local device record for pid=${pid} serial=${serialNumber}`);
		device = new Device({
			model,
			serial: serialNumber,
			environment,
	...
	LOG_INFO(`[NASC][LOCAL-3DS] middleware complete title=${titleID} pid=${nexAccount?.pid || 'none'} device=${device ? 'yes' : 'no'}`);
